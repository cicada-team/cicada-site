...
const loosePartial = (fn, ...args) => (fn === undefined ? noop : partial(fn, ...args))
...
export default function createBackground(backgroundDef = {}, stateTree, appearance) {
  ...
  const registerComponent = (stateId, config, Component) => {
    ...
    const combinedInject = compose(map(filteredUtilities, utility => loosePartial(utility.inject, stateId, config)))
