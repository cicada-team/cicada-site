...
describe('should receive right injected arguments', () => {
  ...
  describe('in listener', () => {
    test('after default listener', () => {
      let receivedArgs = []
      let listenerCalled = false
      const Component = {
        stateTypes: {
      ...
      const container = TestUtils.renderIntoDocument(
        <Render>
          <Connected listeners={{ onClick: { fns: [{ fn: onClick }] } }} />
        </Render>,
      )
      const button = TestUtils.findRenderedDOMComponentWithTag(container, 'button')
      TestUtils.Simulate.click(button)
      ...
      expect(receivedArgs[0].state.clicked).toBe(1)
      TestUtils.Simulate.click(button)
      expect(receivedArgs[0].state.clicked).toBe(2)
    })
  })
